...
app.get('/', (req, res) => {
// turn movies into an oject to be passed to index.ejs
  const data = {
        movie: movies
    };
...
// find movie with :movieId and pass to response
  var display = movies.filter(function (movie) {
...
// datum is the filtered movie converted to an object with one element n the array

    res.render('pages/movie', datum); 

});

 app.get('/search', function (req, res) {
// form is form.ejs note: method="get" action ="/search" and input name = "q"

  var foundMovie = movies.filter(function (movie) {
    return (movie.title).toUpperCase() == (req.query.q).toUpperCase();  
  });
  if (typeof foundMovie[0] !== 'undefined' && foundMovie[0] !== null) {
      const datum = {
      movie: foundMovie
      };
    res.render('pages/search', datum);
